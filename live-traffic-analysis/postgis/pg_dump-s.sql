 ...
 WITH linked_detections AS (
         ...
         SELECT detections.id,
           ...
           FROM (public.detections_extended detections
             LEFT JOIN public.tracked_paths paths ON (((paths.session_id = detections.session_id) AND (paths.tracker_id = detections.tracker_id))))
...
--
-- Name: predictions_snapped; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW public.predictions_snapped AS
 WITH past_and_present_data_snapped AS (
         WITH current_and_past_detections_snapped AS (
                 WITH params AS (
                         SELECT 30 AS lag_value
                        )
                 SELECT detections_2.id,
                    detections_2."timestamp",
                    lag(detections_2."timestamp", ( SELECT params.lag_value
                           FROM params)) OVER w AS timestamp_n_detections_ago,
                    detections_2.session_id,
                    detections_2.tracker_id,
                    detections_2.class_name,
                    detections_2.image_x,
                    detections_2.image_y,
                    detections_2.image_coordinate,
                    public.st_closestpoint(paths.path, detections_2.location) AS location,
                    public.st_closestpoint(paths.path, lag(detections_2.location, ( SELECT params.lag_value
                           FROM params)) OVER w) AS location_n_detections_ago
                   FROM (public.detections_extended detections_2
                     LEFT JOIN public.tracked_paths paths ON (((paths.session_id = detections_2.session_id) AND (paths.tracker_id = detections_2.tracker_id))))
                  WINDOW w AS (PARTITION BY detections_2.session_id, detections_2.tracker_id ORDER BY detections_2."timestamp")
                )
         SELECT detections_1.id,
            detections_1."timestamp",
            detections_1.timestamp_n_detections_ago,
            detections_1.session_id,
            detections_1.tracker_id,
            detections_1.class_name,
            detections_1.image_x,
            detections_1.image_y,
            detections_1.image_coordinate,
            detections_1.location,
            detections_1.location_n_detections_ago,
            public.st_distance(detections_1.location, detections_1.location_n_detections_ago) AS feet_delta,
            EXTRACT(epoch FROM (detections_1."timestamp" - detections_1.timestamp_n_detections_ago)) AS time_delta,
            (public.st_distance(detections_1.location, detections_1.location_n_detections_ago) / (EXTRACT(epoch FROM (detections_1."timestamp" - detections_1.timestamp_n_detections_ago)))::double precision) AS velocity_fps,
            degrees(public.st_azimuth(detections_1.location_n_detections_ago, detections_1.location)) AS angle_of_travel
           FROM current_and_past_detections_snapped detections_1
          WHERE (detections_1.timestamp_n_detections_ago IS NOT NULL)
        )
 SELECT detections.id,
    detections."timestamp",
    detections.timestamp_n_detections_ago,
    detections.session_id,
    detections.tracker_id,
    detections.class_name,
    detections.image_x,
    ...
    detections.image_coordinate,
    detections.location,
    detections.location_n_detections_ago,
    detections.feet_delta,
    detections.time_delta,
    detections.velocity_fps,
    detections.angle_of_travel,
    public.st_makeline(detections.location_n_detections_ago, detections.location) AS path_of_past_travel,
    public.st_transform((public.st_project((public.st_transform(detections.location, 4326))::public.geography, ((detections.velocity_fps * (detections.time_delta)::double precision) * (((1)::numeric / 3.28084))::double precision), radians(detections.angle_of_travel)))::public.geometry, 2253) AS location_future,
    public.st_makeline(detections.location, public.st_transform((public.st_project((public.st_transform(detections.location, 4326))::public.geography, ((detections.velocity_fps * (detections.time_delta)::double precision) * (((1)::numeric / 3.28084))::double precision), radians(detections.angle_of_travel)))::public.geometry, 2253)) AS path_of_future_travel
   FROM past_and_present_data_snapped detections
  WHERE (detections.timestamp_n_detections_ago IS NOT NULL);


ALTER TABLE public.predictions_snapped OWNER TO postgres;
